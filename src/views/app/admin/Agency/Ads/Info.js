...
import { fetchAdAgencyId } from '../../../../../redux/agency/actions';
import BackButtonText from '../../../../../components/BackButtonText';

class Info extends Component {
  componentDidMount() {
    const { id } = this.props.match.params;
    this.props.fetchAdAgency(id);
  }
    ...
    if (features.length > 0) {
      ...
      return features.map(item => (
        <Label className="fullLabel">
          - {constants ? constants.features[item] : item}
        </Label>
      ));
    }
  ...
  renderBuildingFeatures = features => {
    if (features.length > 0) {
      const { constants } = this.props;
      return features.map(item => (
        <Label className="fullLabel">
          - {constants ? constants.building_features[item] : item}
        </Label>
      ));
    }
    return <Label className="fullLabel"> </Label>;
            ...
            <Row>
              ...
              <Col sm="12" md="12" lg="4">
                ...
                {data?.building_features
                  ? this.renderBuildingFeatures(data?.building_features)
                  : null}
              </Col>
            </Row>
            <Row className="align-items-center mt-5">
...
const mapStateToProps = state => ({
  data: state.agency.adDataId,
  loading: state.agency.adFetchIdLoading,
  error: state.agency.adFetchIdError,
  constants: state.ad.constants,
});

const mapActionsToProps = {
  fetchAdAgency: fetchAdAgencyId,
};
